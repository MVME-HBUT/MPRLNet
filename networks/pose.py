...
class GhostNetV2(nn.Module):
    def __init__(self, width=1.0, dropout=0.2,block=GhostBottleneckV2,args=None):
        ...
        [[3, 200,  80, 0, 1],
         ...
         [3, 184,  80, 0, 1],
         [3, 480, 112, 0.25, 1],
         [3, 672, 112, 0.25, 1]#第二个hidden channel 第三个输出channel
        ],
        [[5, 672, 160, 0.25, 2]],
        [[5, 960, 160, 0, 1],
         [5, 960, 160, 0.25, 1],
        ...
        self.conv_stem = nn.Conv2d(6, output_channel, 3, 2, 1, bias=False)
...
class PoseNet(nn.Module):
    def __init__(self):
        ...
        self.encoder = GhostNetV2()
        state_dict = torch.load("ckpt/ck_ghostnetv2_10.pth.tar")
        state_dict['conv_stem.weight'] = torch.cat([state_dict['conv_stem.weight']] * 2, 1) / 2
    ...
    def forward(self, input_image):
        ...
        return_features.append(self.encoder.blocks[0](x))
        return_features.append(self.encoder.blocks[1:3](return_features[-1]))
        return_features.append(self.encoder.blocks[3:5](return_features[-1]))
        return_features.append(self.encoder.blocks[5:7](return_features[-1]))
        return_features.append(self.encoder.blocks[7:](return_features[-1]))
